    ...
    <Card className="lg:col-span-2" padded={false}>
      ...
      <ul className="divide-y divide-[var(--cq-border)]">
        {APPTS.map((a, i) => (
          <li
            key={i}
            className="flex items-center gap-4 px-5 py-3 hover:bg-[var(--cq-surface-2)] transition-colors cursor-pointer group relative"
          >
            {i === NOW_INDEX && (
            ...
            <button
              className="w-7 h-7 rounded-[6px] hover:bg-[var(--cq-surface-3)] opacity-0 group-hover:opacity-100 flex items-center justify-center"
              aria-label="Acciones"
            >
              <Icons.More size={14} />
            </button>
